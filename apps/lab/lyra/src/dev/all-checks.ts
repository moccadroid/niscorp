...
const CHECKS = [
  ...
  ['reads-are-vex-check', 'reads are vex: one licensed pre-auth read, one schema builder, machinery roles pinned, and an engine for everything else'],
  ['identity-sql-check', 'the one licensed statement, held to its artifact: counted, pinned, parameterised, and run against the live schema'],
  ...
  ['stripe-check', 'payments arrive as an integration: declared, placed, framed, and honest about having no key'],
  ['billing-check', 'Stripe speaks and a membership answers: signed, claimed once, stated rather than counted'],
  ['frame-check', 'an integration serves a page and the host frames it — declared, granted, and never identity'],
