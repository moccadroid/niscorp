...
const main = async (): Promise<void> => {
  ...
  await settle(360);
  const goneC = (await runtime.db.query('SELECT count(*)::int AS n FROM contacts WHERE id = $1', [cDel['contact_id']])).rows[0] as { n: number };
  ...
  await settle(360);
  const goneD = (await runtime.db.query('SELECT count(*)::int AS n FROM deals WHERE id = $1', [dId])).rows[0] as { n: number };
  const goneLI = (await runtime.db.query('SELECT count(*)::int AS n FROM deal_products WHERE deal_id = $1', [dId])).rows[0] as { n: number };
  ...
  const withChildren = (await runtime.db.query(`
  ...
  const kidsBefore = (await runtime.db.query('SELECT (SELECT count(*) FROM contacts WHERE company_id=$1)::int AS c, (SELECT count(*) FROM deals WHERE company_id=$1)::int AS d', [withChildren.id])).rows[0] as { c: number; d: number };
  shell.dispatch({ type: 'ui:click', ref: 'row-delete', payload: coRow });
  await settle(220);
  shell.dispatch({ type: 'ui:click', ref: 'confirm' });
  await settle(380);
  const after = (await runtime.db.query('SELECT (SELECT count(*) FROM companies WHERE id=$1)::int AS co, (SELECT count(*) FROM contacts WHERE company_id=$1)::int AS c, (SELECT count(*) FROM deals WHERE company_id=$1)::int AS d', [withChildren.id])).rows[0] as { co: number; c: number; d: number };
