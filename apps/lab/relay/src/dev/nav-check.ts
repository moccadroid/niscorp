...
const main = async (): Promise<void> => {
  ...
  await settle();
  checks.push(['sidebar sets the contacts root', activeId(), 'contacts'], ['root is depth 1', depth(), 1]);

  // Row drills into a contact (pushed onto main).
  shell.dispatch({ type: 'ui:click', ref: 'row', payload: 'con_001' });
  await settle();
  checks.push(['row drills into the contact', activeId(), 'contact'], ['depth 2', depth(), 2]);
  ...
  await settle();
  checks.push(['cross-link drills into the company', activeId(), 'company'], ['depth 3', depth(), 3]);
  ...
  await settle();
  checks.push(['breadcrumb jumps back to the contact', activeId(), 'contact'], ['popped to depth 2', depth(), 2]);

  // The chip's Back pops one → the contacts list root.
  shell.pop('main');
  await settle();
  checks.push(['Back returns to the contacts list', activeId(), 'contacts'], ['depth 1 again', depth(), 1]);
