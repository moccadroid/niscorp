...
const main = async (): Promise<void> => {
  ...
  const r1 = await stackTool.config.execute({ canvas: 'main', op: 'push', action: 'deals', input: { view: 'board' } }, ctx);
  await settle(320);
  checks.push([`push deals board on main (got ${String(activeId('main'))} view=${String(activeData('main')['view'])})`, activeId('main') === 'deals' && activeData('main')['view'] === 'board']);
  ...
  await stackTool.config.execute({ canvas: 'modal', op: 'push', action: 'deal.form', input: { title: 'Ray Test Deal' } }, ctx);
  await settle(220);
  ...
  await settle(220);
  checks.push([`pop main returns to deals (got ${String(activeId('main'))})`, activeId('main') === 'deals']);
