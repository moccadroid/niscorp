...
import { createGroqClient, getKey as getGroqKey } from '../../llm/groq';
...
export const runActionArchitect = async (
  ...
  const tools = makeArchitectTools(supportLlm);
  ...
  // Context debugging: `localStorage.setItem('relay.ray.debug-context', '1')`
  // → every architect run prints its EXACT assembled prefix (the same
  // sections the model sees, via agent.preview) before running. The
  // node-side twin is src/dev/architect-preview.ts.
  if (lsGet('relay.ray.debug-context') !== null) {
    const preview = await architectAgent.preview({ intent: architectInput }, {
      llm: agentLlm,
      tools: [tools.discover, tools.query, tools.runAction],
      ...(opts.base !== undefined && { producers: [editingGuide] }),
    });
    console.log(`[architect context] strategy=${preview.strategy} ~${preview.estimatedTokens} tokens, ${preview.messages.length} messages`);
    for (const [index, message] of preview.messages.entries()) {
      const content = typeof message.content === 'string' ? message.content : JSON.stringify(message.content);
      console.log(`[architect context] #${index} ${message.role} (${content.length} ch)\n${content}`);
    }
  }
...
export const makeBuildActionTool = (shell: Shell): ToolDefinition => {
  ...
  return defineTool({
    ...
    execute: async ({ intent, edit }, ctx) => {
      ...
      if (key === undefined) return 'No Groq key set — add one with the 🔑 button before building.';
      ...
      let buildResult = await runActionArchitect(llm, llm, intent, {
        ...pipelineOpts,
        ...(base !== undefined && { base }),
      });
      ...
      let report = await runAction(builtAction, undefined, buildResult.proofs);
      ...
      if (firstBlockers.length > 0) {
        const repairResult = await runActionArchitect(llm, llm, `Fix these findings:\n${firstBlockers.join('\n')}`, {
          ...pipelineOpts,
          base: builtAction,
        });
        if (repairResult.ok) {
          builtAction = { ...namespaceActionId(repairResult.action), id: builtAction.id };
          report = await runAction(builtAction, undefined, repairResult.proofs);
      ...
      shell.registerAction(builtAction);
      ...
      shell.push('main', builtAction.id);
