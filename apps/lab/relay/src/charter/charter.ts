...
// action ids, `data` selects `table.verb` capabilities that compile to a vex
// ScopePolicy (see policy.ts). `extends` composes both sections at once.
//
...
export const CHARTER: Charter = {
  ...
  sales: {
    extends: ['viewer'],
    actions: ['crm.*', 'tasks.*', 'assistant', 'keys'],
