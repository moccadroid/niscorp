...
// there stay responsive and fill whatever the canvas gives them. Swappable via
// shell.setLayout('main', …) without touching the sidebar/topbar (those are in
// the frame, not here).
export const mainSplitLayout: LayoutNode = {
  // `grow` (flex:1) fills the height the topbar LEFT — not `h:100%`, which would
  // be the full 100vh and overflow. `shrink` lets it (and the scroll Box) shrink
