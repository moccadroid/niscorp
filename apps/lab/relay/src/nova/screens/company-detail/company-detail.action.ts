...
import { companyDetailLayout } from './company-detail.layout';
...
export const companyDetailAction: ActionDefinition = {
  id: 'company-detail',
  data: { id: '', view: { record: {}, contacts: [], deals: [] }, loading: true },
  layout: companyDetailLayout,
  endpoints: {
  ...
  triggers: [
      ...
      do: [
        {
          push: {
            ...
            canvas: 'modal',
            with: ['modal'],
            input: { id: '$.view.record.company_id', name: '$.view.record.name', domain: '$.view.record.domain', industry: '$.view.record.industry', size: '$.view.record.size' },
          },
        },
      ],
    },
    { message: 'companies-changed', do: [{ call: 'loadRecord' }] },
    {
      ...
      do: [
        ...
        { replace: { action: 'contact-detail', canvas: 'detail', input: { id: '@event.payload' } } },
      ],
    },
    // Open one of the company's deals in the deal workspace (the one deal view),
    // as a modal over this panel.
    { event: 'ui:click', ref: 'open-deal', do: [{ push: { action: 'deal-modal', canvas: 'modal', input: { id: '@event.payload' } } }] },
  ],
};
