...
export const companiesAction: ActionDefinition = {
  ...
  triggers: [
    ...
    { event: 'ui:click', ref: 'row', do: [{ set: 'highlight_id', value: '@event.payload' }, { replace: { action: 'company-detail', canvas: 'detail', input: { id: '@event.payload' } } }] },
    { message: 'deselect', do: [{ set: 'highlight_id', value: '' }] },
    { message: 'new', do: [{ push: { action: 'new-company', canvas: 'modal', with: ['modal'] } }] },
    ...
    { event: 'ui:click', ref: 'row-open', do: [{ set: 'highlight_id', value: '@event.payload.company_id' }, { replace: { action: 'company-detail', canvas: 'detail', input: { id: '@event.payload.company_id' } } }, { set: 'menuOpenId', value: '' }] },
    {
      event: 'ui:click',
      ref: 'row-edit',
      do: [
        { set: 'menuOpenId', value: '' },
        { push: { action: 'edit-company', canvas: 'modal', with: ['modal'], input: { id: '@event.payload.company_id', name: '@event.payload.name', domain: '@event.payload.domain', industry: '@event.payload.industry', size: '@event.payload.size' } } },
      ],
    },
    {
      event: 'ui:click',
