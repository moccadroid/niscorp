...
export const contactsAction: ActionDefinition = {
  ...
  triggers: [
    ...
    { event: 'ui:click', ref: 'row', do: [{ set: 'highlight_id', value: '@event.payload' }, { replace: { action: 'contact-detail', canvas: 'detail', input: { id: '@event.payload' } } }] },
    { message: 'deselect', do: [{ set: 'highlight_id', value: '' }] },
    { message: 'new', do: [{ push: { action: 'new-contact', canvas: 'modal', with: ['modal'] } }] },
    ...
    { event: 'ui:click', ref: 'row-open', do: [{ set: 'highlight_id', value: '@event.payload.contact_id' }, { replace: { action: 'contact-detail', canvas: 'detail', input: { id: '@event.payload.contact_id' } } }, { set: 'menuOpenId', value: '' }] },
    {
      event: 'ui:click',
      ref: 'row-edit',
      do: [
        { set: 'menuOpenId', value: '' },
        { push: { action: 'edit-contact', canvas: 'modal', with: ['modal'], input: { id: '@event.payload.contact_id', name: '@event.payload.name', email: '@event.payload.email', phone: '@event.payload.phone', title: '@event.payload.title', company: '@event.payload.company.company_id' } } },
      ],
    },
    {
      event: 'ui:click',
