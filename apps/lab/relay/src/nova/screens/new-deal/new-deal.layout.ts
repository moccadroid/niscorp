...
// The deal form body — wrapped by the `modal` fragment. Bound to the action data
// (the FORM's shape). Company / Stage / Primary contact are id-bearing selects:
// options load from `options.companies` / `options.stages` / `options.contacts`
// (entity-distinct shapes), and the Select reads each via valueKey/labelKey, so
// `$.company`/`$.stage`/`$.contact` hold real FK ids — the create writes them
// directly. Shared by new-deal (create) and edit-deal (edit). Literal +
// serializable.
export const newDealLayout: LayoutNode = {
  component: 'Stack',
  props: { gap: 13 },
  children: [
