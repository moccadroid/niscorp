...
// The deal workspace — the ONE deal view — is several reads into slots of
// `$.view`. The primary contact keys off `$.view.record.primary_contact_id`, so
// its endpoint runs after the record loads (see the action's mount sequence).
export const dealModalPrism: Record<string, unknown> = {
...
export const dealModalMutations: Record<string, unknown> = {
