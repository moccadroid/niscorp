...
export const tasksAction: ActionDefinition = {
  ...
  triggers: [
    ...
    // `new` (the topbar's + New) opens the new-task modal.
    { message: 'new', do: [{ push: { action: 'new-task', canvas: 'modal', with: ['modal'] } }] },
    ...
    { event: 'ui:click', ref: 'menu-close', do: [{ set: 'menuOpenId', value: '' }] },
    {
      ...
      do: [
        ...
        { push: { action: 'edit-task', canvas: 'modal', with: ['modal'], input: { id: '@event.payload.task_id', title: '@event.payload.title', due: '@event.payload.due_date' } } },
      ],
    },
    {
      event: 'ui:click',
