...
export const dealAction: ActionDefinition = {
  ...
  triggers: [
    ...
    // Edit opens the deal form seeded for UPDATE from the RAW record fields (value
    // is the number; close_date/stage/company/contact are the raw ids). `saveFn`
    // switches the form's `save` endpoint to `deal.update`; `deals-changed` (the
    // save announces it) re-reads this record.
    {
      event: 'ui:click',
      ref: 'edit',
      do: [
        {
          push: {
            ...
            canvas: 'modal',
            with: ['modal'],
            input: { saveFn: 'deal.update', modalTitle: 'Edit deal', confirmLabel: 'Save', id: '$.view.record.deal_id', title: '$.view.record.title', company: '$.view.record.company_id', stage: '$.view.record.stage_id', contact: '$.view.record.primary_contact_id', value: '$.view.record.value', close_date: '$.view.record.close_date' },
          },
        },
      ],
    },
