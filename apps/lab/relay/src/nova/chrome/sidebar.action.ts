...
export const sidebarAction: ActionDefinition = {
  ...
  triggers: [
    { event: 'ui:click', ref: 'nav-home', do: [{ replace: { action: 'home', canvas: 'main' } }, { emit: { channel: 'nav' } }, { emit: { channel: 'screen-home' } }] },
    { event: 'ui:click', ref: 'nav-tasks', do: [{ replace: { action: 'tasks', canvas: 'main' } }, { emit: { channel: 'nav' } }, { emit: { channel: 'screen-tasks' } }] },
    { event: 'ui:click', ref: 'nav-pipeline', do: [{ replace: { action: 'deals', canvas: 'main', input: { view: 'board' } } }, { emit: { channel: 'nav' } }, { emit: { channel: 'screen-pipeline' } }] },
    { event: 'ui:click', ref: 'nav-contacts', do: [{ replace: { action: 'contacts', canvas: 'main' } }, { emit: { channel: 'nav' } }, { emit: { channel: 'screen-contacts' } }] },
    { event: 'ui:click', ref: 'nav-companies', do: [{ replace: { action: 'companies', canvas: 'main' } }, { emit: { channel: 'nav' } }, { emit: { channel: 'screen-companies' } }] },
    { event: 'ui:click', ref: 'nav-deals', do: [{ replace: { action: 'deals', canvas: 'main', input: { view: 'table' } } }, { emit: { channel: 'nav' } }, { emit: { channel: 'screen-deals' } }] },
    { event: 'ui:click', ref: 'nav-settings', do: [{ replace: { action: 'settings', canvas: 'main' } }, { emit: { channel: 'nav' } }, { emit: { channel: 'screen-settings' } }] },
