...
const fakeFetch: FetchFn = async (url) => {
  ...
  if (url.includes('/broken')) {
    const body = '{"message":"Internal server error from mock"}';
    return {
      ok: false,
      status: 500,
  ...
  const body = '{"id":"u_001","name":"Ada Lovelace","savedAt":"2026-04-07"}';
  return {
    ok: true,
    status: 200,
...
export const endpointFullStory: ActionStory = {
  id: 'endpoint-full',
  name: 'Endpoint (full)',
  description:
    'Full endpoint demo: templated URL, templated headers, templated body, success/error chains, '
    + 'loading state, and conditional success/error boxes. The "Save (force fail)" button calls a '
    + 'broken endpoint so the onError chain reads `@error.message`.',
  kind: 'action',
  category: 'Endpoints',
  action: {
    id: 'endpoint-full',
    ...
    layout: {
      ...
      children: [
        ...
        {
          if: '$.loading',
          then: { component: 'Text', children: 'Saving...' },
        },
        {
          if: '$.success',
          then: {
  ...
  fetch: fakeFetch,
  expected: { textIncludes: ['Save user', 'User ID', 'Name', 'Email', 'Save'] },
};
