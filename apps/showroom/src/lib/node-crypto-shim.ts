...
// shape/request/schema hashes — a Node builtin that doesn't exist
// in the browser. Vite aliases `node:crypto` to this module (see
// vite.config.ts).
//
// Backed by @noble/hashes, whose SHA-256 is byte-identical to
// Node's, so a shape hash computed by the Node prewarm step and one
// computed here in the browser MATCH — which is what lets the
// prewarmed cache entries resolve. Anything beyond
// createHash('sha256') is intentionally unsupported and throws, so
// an unexpected new dependency on node:crypto surfaces loudly
